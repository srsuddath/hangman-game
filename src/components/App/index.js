...
class App extends Component {
  ...
  generateChallengeDisplay = (hint, challengeSolution) => {
    ...
    let challenge = challengeSolution.map((letter) =>
      letter.replace(/[A-z0-9]/g, "_")
    );
  ...
  generatePuzzle = async () => {
    ...
    try {
      ...
      this.generateChallengeDisplay(
        parsedResponse.setup,
        parsedResponse.punchline
      );
    } catch (error) {
  ...
  guess = () => {
    ...
    if (
      guessedCorrectLetters.includes(guess) ||
      guessedIncorrectLetters.includes(guess)
    ) {
    ...
    if (guessIsCorrect) {
      this.setState({
        guessedCorrectLetters: [...guessedCorrectLetters, guess],
        challenge,
        guess: "",
      });
      this.checkForWin();
    ...
    this.setState(
      {
        ...
        guessedIncorrectLetters: [...guessedIncorrectLetters, guess],
        challenge,
        guess: "",
      },
      () => this.checkForLoss()
  ...
  deriveChallengeDisplay = (challenge) => {
    return challenge.map((letter, index) => {
      if (letter === " ") {
        return (
          <span className="space" key={letter + index}>
            {letter}
          </span>
        );
      }
      if (
        ...
        return (
          <span className="punctuation" key={letter + index}>
            {letter}
          </span>
        );
      }
  ...
  render() {
    ...
    return (
      <div className="app">
        ...
        {puzzleSolved && (
          <h1 className="victorySign">YOU SOLVED THE PUZZLE!</h1>
        )}

        {hint && !puzzleSolved && (
          <h3 className="lifeCounter">{this.deriveLifeCounterText()}</h3>
        )}

        {!puzzleSolved && (
          <div className="puzzle">
            {hint && <h2>{this.deriveChallengeDisplay(challenge)}</h2>}
        ...
        {puzzleSolved && (
          <div className="puzzle">
            <h3>{hint}</h3>
            <h2>{this.deriveChallengeDisplay(challenge)}</h2>
            <br />
          </div>
        )}
        ...
        <div className="inputs">
              ...
              {numbers.split("").map((number) => {
                return (
                  <button
                    ...
                    type="button"
                    onClick={
                      guessedCorrectLetters.includes(number) ||
                      guessedIncorrectLetters.includes(number)
                        ? ""
              ...
              {keyboard1.split("").map((letter) => {
                return (
                  <button
                    ...
                    key={letter}
                    onClick={
                      guessedCorrectLetters.includes(letter) ||
                      guessedIncorrectLetters.includes(letter)
                        ? ""
                        : () => this.handleButtonClick(letter)
                    }
                    type="button"
                  >
                    {letter}
          ...
          {hint && !puzzleSolved && !puzzleFailed && (
            <div>
              <input
                className="guess-box"
                type="text"
